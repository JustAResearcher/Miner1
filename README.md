...
- `bc3miner-hiveos-0.1.3_rc2.tar.gz` for HiveOS testing on Turing and Ampere
  GPUs, including compute capability 8.0 devices such as CMP 170HX.

The CatStack bundle supports its validated RTX 40/50 fleet. The v0.1.3 RC2
HiveOS package supports SM 7.5, 8.0, and 8.6 hardware; it is not a replacement
for v0.1.2. CatStack's built-in BC3Miner definition downloads the first bundle
...
- Compute capability 8.0 / Ampere datacenter and mining GPUs: legacy solver.
...
- NVIDIA compute capability 7.5, 8.0, and/or 8.6
...
Build the Turing/Ampere release candidate with its checksum-pinned combined
legacy payload:

```bash
bash scripts/build-hiveos-package.sh \
  --sm75-sm80-sm86 /path/to/bc3miner \
  --sm75-sm80-sm86-sha256 HASH_FROM_SHA256SUM
bash tests/test-hiveos-package.sh dist/bc3miner-hiveos-0.1.3_rc2.tar.gz
```

Outputs:

- `dist/bc3miner-hiveos-0.1.3_rc2.tar.gz`
- `dist/bc3miner-hiveos-0.1.3_rc2.tar.gz.sha256`
...
| Installation URL | `https://github.com/JustAResearcher/Miner1/releases/download/v0.1.3-rc2/bc3miner-hiveos-0.1.3_rc2.tar.gz` |
