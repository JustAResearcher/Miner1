...
readonly PACKAGE_VERSION=0.1.3_rc1
...
usage() {
    cat >&2 <<EOF
Usage:
  $0 --sm75-sm86 PATH --sm75-sm86-sha256 HASH

Builds the HiveOS RTX 20/30 release candidate from one checksum-pinned
legacy solver binary containing native sm_75 and sm_86 cubins.
EOF
}
...
[[ -f $payload ]] || { printf 'build: --sm75-sm86 must name a regular file\n' >&2; exit 2; }
[[ $payload_sha256 =~ ^[0-9a-f]{64}$ ]] || {
    printf 'build: --sm75-sm86-sha256 must be 64 hexadecimal characters\n' >&2
    exit 2
}

payload_copy="$work_dir/bc3miner-sm75-sm86"
...
install -d -m 0755 "$stage_dir/lib/sm75-sm86"
...
install -m 0755 "$payload_copy" "$stage_dir/lib/sm75-sm86/bc3miner"

sed -i \
    -e "s|^readonly SM75_SM86_SHA256=.*|readonly SM75_SM86_SHA256=\"$payload_sha256\"|" \
    "$stage_dir/bc3miner"
sed -i \
    -e "s|__SM75_SM86_SHA256__|$payload_sha256|g" \
    "$stage_dir/BUILD_INFO.json"
printf '%s  %s\n' "$payload_sha256" 'lib/sm75-sm86/bc3miner' > "$stage_dir/PAYLOAD_SHA256SUMS"

(
    cd "$stage_dir"
    sha256sum -c PAYLOAD_SHA256SUMS
    bash -n h-config.sh h-run.sh h-stats.sh bc3miner
    grep -Fqx "readonly SM75_SM86_SHA256=\"$payload_sha256\"" bc3miner
